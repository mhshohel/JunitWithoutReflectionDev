 ...
 * HREF="http://www.cat.nyu.edu/meyer/jasmin"> JASMIN</A> format.
 * 
...
public class JasminVisitor extends EmptyVisitor {
	...
	printEndMethod((Attribute) attribute);
    }
	...
	out.println("\n.method "
		+ Utility.accessToString(method.getAccessFlags()) + " "
	...
	for (int i = 0; i < lvs.length; i++) {
	    ...
	    out.println(".var " + l.getIndex() + " is " + l.getName() + " "
		    + l.getType().getSignature() + " from " + get(l.getStart())
		    + " to " + get(l.getEnd()));
	}
	...
	for (int i = 0; i < ihs.length; i++) {
		...
		if (inst instanceof Select) { // Special cases LOOKUPSWITCH and
		    ...
		    out.println("\t\tdefault: " + get(s.getTarget())); // Applies
								       // for
								       // both
		} else {
	...
	for (int i = 0; i < ehs.length; i++) {
	    ...
	    out.println(".catch " + class_name + " from " + get(c.getStartPC())
		    + " to " + get(c.getEndPC()) + " using "
		    + get(c.getHandlerPC()));
	}
	...
	if (str == null)
	    ...
	    if (line.startsWith("Label") || str.endsWith(line)) // Already have
								// a label in
								// the map
		return;
		...
		for (int i = 0; i < argv.length; i++) {
		    ...
		    FileOutputStream out = new FileOutputStream(
			    "G:\\lnu\\5DV001 - Thesis Project\\Thesis - Jonas\\ThesisBackup\\JunitWithoutReflection.git.first\\JunitTestWithActualLib\\bin\\callgraphstat\\"
				    + class_name + ".j");
		    JasminVisitor jv = new JasminVisitor(java_class, out);
		    jv.disassemble();
		    // jv.visitJavaClass(java_class);
		}
	    }
	} catch (Exception e) {
	    e.printStackTrace();
